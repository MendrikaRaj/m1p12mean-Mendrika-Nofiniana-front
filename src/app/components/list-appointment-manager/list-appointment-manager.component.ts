        ...
        this.appointments =
          response.map((appointment: any) => {
            ...
            this.authService.getUserById(appointment.client_id).subscribe({
              next: (user) => {
                appointment.clientDetails = user;
              },
              error: (error) => {
                console.error(
                  `Error retrieving user with ID ${appointment.client_id}:`,
                  error
                );
                appointment.clientDetails = null;
              },
            });

            // Check if appointment exists in service-auto using getAllService
            this.serviceAutoService.getAllServices().subscribe({
              next: (services: any[]) => {
              appointment.existsInServiceAuto = services.some(
                (service) => service.rendez_vous_id === appointment._id
              );
              },
              error: (error) => {
              console.error(
                `Error retrieving services for appointment ${appointment._id}:`,
                error
              );
              appointment.existsInServiceAuto = false;
              },
            });
